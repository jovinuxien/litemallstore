...
#if [[ "$TYPE" == "json" ]] && [[ "$MAPPING" == "" ]]; then
#  echo "JSON file structure preview:"
#  head -n1 "$FILE" | jq '. | "First object has keys: " + (keys | join(", "))'
#  echo ""
#  echo "Sample of first object."
#  head -n1 "$FILE" | jq '.'
#  exit 0
#fi
...
# Json section
    ...
    # Use a smaller batch size to avoid "Argument list too long"
    BATCH_SIZE=20
    ...
    while read -r jsonline; do
        ...
        if [[ "$bulk_size" -eq "$BATCH_SIZE" ]]; then
