...
public class LitemallGoods {
    ...
    /**
     * Origin of this goods row: 'local' (hand-authored) or 'cj' (synced from CJ Dropshipping).
     * Added by V23__cj_native_linkage; see also {@link #cjPid}.
     */
    private String source;

    /**
     * CJ Dropshipping product id (UUID) when {@link #source} = 'cj'; null for local goods.
     * Together with source it forms the unique key used for idempotent CJ upserts.
     */
    private String cjPid;

    /**
     * Ranking signals for OCS relevance boosting (V31). {@code listedNum} = CJ platform
     * popularity (merchant listing count; 0 for local); {@code reviewCount}/{@code rating} =
     * unified review aggregate (local litemall_comment or CJ productComments). Read by
     * createProductDocument, indexed as numeric Sort+Score fields.
     */
     ...
     * Corresponds to litemall_goods.temp_id
     */
    private Integer tempId;

    /**
     * Hand-maintained (Wave 4): freight template binding / billing units; columns exist
     * since V2. Weight in kg (billing unit when the bound template is by-weight).
     * Corresponds to litemall_goods.weight
     */
    private BigDecimal weight;

    /**
     * Hand-maintained (Wave 4): freight template binding / billing units; columns exist
     * since V2. Volume in m3 (billing unit when the bound template is by-volume).
     * Corresponds to litemall_goods.volume
     */
    private BigDecimal volume;

    /**
     * Hand-maintained (Wave 12): landed wholesale cost (CJ sell_price, USD basis); column
     * exists since V2 (NOT NULL DEFAULT 0.00). 0.00 means "not captured yet" — margin
     * consumers must render null then, never 0%.
     * Corresponds to litemall_goods.cost
     */
    ...
    public void setCjPid(String cjPid) {
        this.cjPid = cjPid;
    }
    ...
    public static enum Column {
        ...
        cjPid("cj_pid", "cjPid", "VARCHAR", false),

        tempId("temp_id", "tempId", "INTEGER", false),
