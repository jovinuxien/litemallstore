...
public class LitemallOrder {
    ...
    /**
     * Hand-maintained (V27): litemall_address.id the order was submitted with, so the
     * pay-time CJ placement can re-resolve the structured shipping address.
     * Corresponds to litemall_order.address_id
     */
    private Integer addressId;

    /**
     * Hand-maintained (V27): ISO destination country picked at checkout (the address
     * book stores no country; CJ createOrder requires one).
     * Corresponds to litemall_order.country_code
     */
    private String countryCode;

    /**
     * Hand-maintained (V27): fulfillment origin of the order: 'local' | 'cj'.
     * Corresponds to litemall_order.source
     */
    private String source;

    /**
     * Hand-maintained (V27): CJ Dropshipping order id returned by createOrder (source=cj).
     * Corresponds to litemall_order.cj_order_id
     */
    private String cjOrderId;

    /**
     * Hand-maintained (V27): CJ Dropshipping order number returned by createOrder (source=cj).
     * Corresponds to litemall_order.cj_order_num
     */
    private String cjOrderNum;

    /**
     * Hand-maintained (V33): last CJ order status seen by the lifecycle sync
     * (CREATED/IN_CART/UNPAID/UNSHIPPED/SHIPPED/DELIVERED/CANCELLED), source=cj only.
     * Corresponds to litemall_order.cj_order_status
     */
    ...
    /**
     * Hand-maintained (V35): in-store pickup / write-off. litemall_store.id the
     * customer picked (delivery_type=pickup only).
     * Corresponds to litemall_order.store_id
     */
    private Integer storeId;

    /**
     * Hand-maintained (V35): in-store pickup / write-off. Pickup write-off code,
     * generated at pay time; UNIQUE across orders.
     * Corresponds to litemall_order.verify_code
     */
    private String verifyCode;

    /**
     * Hand-maintained (V35): in-store pickup / write-off. When the order was written off.
     * Corresponds to litemall_order.verify_time
     */
    private LocalDateTime verifyTime;

    /**
     * Hand-maintained (V35): in-store pickup / write-off. Who wrote the order off,
     * e.g. admin:&lt;id&gt;.
     * Corresponds to litemall_order.verified_by
     */
    private String verifiedBy;

    /**
     * Hand-maintained (V43): the verified Stripe PaymentIntent id (pi_...). Distinct from
     * pay_id, which is an overloaded tender label ('WALLET', 'CREDIT_CARD:pi_...',
     * 'OFFLINE:...') and therefore cannot be unique. NULL for wallet/offline tenders;
     * UNIQUE across orders, which is what makes a PaymentIntent replay onto a second
     * order impossible rather than merely unlikely.
     * Corresponds to litemall_order.payment_intent_id
     */
    private String paymentIntentId;

    /**
     * Hand-maintained (V43): tax collected at checkout (US sales tax / EU VAT). 0.00 when
     * tax is disabled, never null — the money chain sums it.
     * Corresponds to litemall_order.tax_price
     */
    private BigDecimal taxPrice;

    /**
     * Hand-maintained (V43): provider tax breakdown JSON (per-jurisdiction lines), kept
     * for invoices and audit. Never summed back into the total.
     * Corresponds to litemall_order.tax_breakdown
     */
    ...
    public void setCjOrderId(String cjOrderId) {
        this.cjOrderId = cjOrderId;
    }

    public String getCjOrderNum() {
        return cjOrderNum;
    }

    public void setCjOrderNum(String cjOrderNum) {
        this.cjOrderNum = cjOrderNum;
    }

    public String getCjOrderStatus() {
        return cjOrderStatus;
    }

    public void setCjOrderStatus(String cjOrderStatus) {
    ...
    public String toString() {
        ...
        sb.append(", paymentIntentId=").append(paymentIntentId);
        sb.append(", taxPrice=").append(taxPrice);
        sb.append(", taxBreakdown=").append(taxBreakdown);
        sb.append("]");
        return sb.toString();
    }
    ...
    public static enum Deleted {
        NOT_DELETED(Boolean.valueOf("0"), "未删除"),
        IS_DELETED(Boolean.valueOf("1"), "已删除");
    ...
    public static enum Column {
        ...
        cjOrderId("cj_order_id", "cjOrderId", "VARCHAR", false),
        ...
        verifyCode("verify_code", "verifyCode", "VARCHAR", false),

        verifyTime("verify_time", "verifyTime", "TIMESTAMP", false),

        verifiedBy("verified_by", "verifiedBy", "VARCHAR", false),

        paymentIntentId("payment_intent_id", "paymentIntentId", "VARCHAR", false),

        taxPrice("tax_price", "taxPrice", "DECIMAL", false),

        taxBreakdown("tax_breakdown", "taxBreakdown", "LONGVARCHAR", false);
