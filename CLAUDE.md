...
## ACTIVE WORKTREE — Phase 5: Frontend split, retire all-react-war

Branch `refactor/phase-5-frontend-split`.

- Move customer SPA → `litemall-gateway-api/src/main/webapp`; admin SPA
  stays in `litemall-gateway-admin/src/main/webapp`.
- Introduce an npm workspace shared package for common FE code.
- Two separate Redux stores; retire the combined
  `loginUserThunk` + `loginAdminThunk` auth slice (no shared auth slice).
- Wire `frontend-maven-plugin` into `litemall-gateway-api` (admin already
  has it).
- Remove `litemall-all-react-war` from root `pom.xml` and delete the module.

**Verify:** both SPAs build via their gateway's `frontend-maven-plugin`;
root reactor build no longer references `litemall-all-react-war`.