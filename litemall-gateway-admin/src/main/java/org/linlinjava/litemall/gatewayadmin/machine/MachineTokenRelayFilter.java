 ...
 *
 * <p>For every {@code lb://} route the admin edge sets {@code Authorization:
 * Bearer <machine-token>} on the proxied request, replacing the removed
 * Keycloak {@code TokenRelay}. The end-user identity is carried separately as
 * {@code X-User-*} by {@code IdentityForwardingFilter}; downstream services
 * (litemall-svcsecurity) trust those headers only behind a valid machine
 * token. Non-{@code lb} routes (e.g. direct http) are left untouched.
 */
@Component
public class MachineTokenRelayFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        OAuth2AuthorizeRequest request = OAuth2AuthorizeRequest
                ...
                .principal("gateway-admin-machine")
                .build();
