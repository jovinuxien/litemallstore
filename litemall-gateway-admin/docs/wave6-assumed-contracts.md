...
|---|---|---|
| Promote composer (goods list/detail) | **RECONCILED 2026-07-16 against promotion's committed `docs/handoff-social-composer.md`** (verified live on :18081 the same day): preview carries top-level `price/dealPrice/dealId` and PER-PLATFORM `shareUrl`; post → `{results:[{platform, postId, status, externalPostId, error}]}`; 400 bodies are `{error}`. One deliberate divergence from the handoff's UI hint: the TikTok checkbox is also disabled while its adapter is disabled (the CLAUDE.md Wave-6 block asks for exactly that); Meta stays selectable per the handoff (honest failed-row path) | `app/shared/reducers/private/services/adminSocialApi.ts` |
| Social posts page (`/admin/promotion/social`) | Same reconciliation: list → `{total, page, limit, list[]}` (no `pages`); rows add `dealId`; retry → 200 result DTO (may itself report `failed` — surfaced), 400 `{error}` on non-failed rows, 404 unknown id | `adminSocialApi.ts` |
