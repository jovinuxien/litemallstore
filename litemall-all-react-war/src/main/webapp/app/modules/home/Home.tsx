...
const HomeView: React.FC<Props> = ({ categoriesListHome, entities }) => {
  ...
  const allFloorGoods: IGood[] = entities?.floorGoodsList.reduce((acc, floorGood) => [...acc, ...floorGood.goodsList], []);
    ...
    <div className='groupon-style-home'>
      ...
      <Container>
        ...
        <section>
          ...
          <Row>
            {dataCategoryIndex.categoryList.slice(0, 6).map(category => (
              <Col key={category.id} xs={6} sm={4} md={3} lg={2} className='mb-4'>
                <Card className='text-center h-100 shadow-sm'>
                  ...
                  <Card.Footer>
                    <Link to={`/category/${category.id}`} key={category.id} className='btn btn-sm btn-outline-primary'>
                      Shop Now
                    </Link>
                  </Card.Footer>
                </Card>
