...
public class LitemallCJProductController {
    ...
    @GetMapping("/producdtCJByCategory")
    public Object getProductByCategory(){
        // Get products from specific categorySecondNames across all categoryFirstNames
        List<String> categorySecondNames = Arrays.asList(
                "Tops & Sets", "Bottoms", "Outerwear & Jackets",
                "Weddings & Events", "Pet Apparels", "Pet Collars, Harnesses & Accessories"
        );

        // Or get products from specific categoryFirstNames only
        List<String> categoryFirstNames = Arrays.asList("Women's Clothing", "Pet Supplies");
        //CJProductDataResponse response = productService.getProductsByTopCategoryNames(categoryFirstNames, null);

        var productResponse = productService.getProductsByTopCategoryNames(null, categorySecondNames);
        Map<String, Object> data = new HashMap<>();
        data.put("code", productResponse.getCode());
        data.put("result", productResponse.isResult());
        data.put("message", productResponse.getMessage());
